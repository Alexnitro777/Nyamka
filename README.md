...
> Краткий обзор всех возможностей — в [`docs/features.md`](docs/features.md). Подробный разбор всех сценариев верификации и апелляций — в [`docs/verification-and-appeals.md`](docs/verification-and-appeals.md). Справочник по правам доступа — в [`docs/permissions.md`](docs/permissions.md).
...
│   │   └── sync_ages.ts       /синхронизироватьвозраст (owner-id) — выдать роли возраста уже одобренным
│   ├── buttons/               Кнопки
...
1. **Клонируй репозиторий:**
    ...
    ```bash
    git clone https://github.com/Alexnitro777/Nyamka.git
    cd Nyamka
    ```
...
4. **Настрой серверы.** Конфигурации в файле нет — настройки каждого сервера лежат в таблице `guild_settings` (по строке на каждый ключ). Шаги ниже повтори для **каждого** сервера, где работает бот.
    ...
    **4.4. Вставь строки в `guild_settings`** через SQL или GUI (phpMyAdmin / Adminer / DBeaver). Ключи — с точками; `roles.staff` и `roles.ststaff` — это списки ID ролей через запятую. Замени плейсхолдеры на свои ID:
    ...
    ('ID_СЕРВЕРА', 'roles.age13_15',        'ID_РОЛИ'),
    ('ID_СЕРВЕРА', 'roles.age16_17',        'ID_РОЛИ'),
    ('ID_СЕРВЕРА', 'roles.age18_20',        'ID_РОЛИ'),
    ('ID_СЕРВЕРА', 'roles.age21plus',       'ID_РОЛИ'),
    ...
    | `roles.age13_15` | нет | Роль возраста 13-15 лет |
    | `roles.age16_17` | нет | Роль возраста 16-17 лет |
    | `roles.age18_20` | нет | Роль возраста 18-20 лет |
    | `roles.age21plus` | нет | Роль возраста 21+ лет |
